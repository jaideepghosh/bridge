import { Collection, Environment, ActiveTab, KeyValuePair, AuthConfig, Folder } from "@/types";
...
function mergeKVArrays(base: KeyValuePair[], override: KeyValuePair[]): KeyValuePair[] {
  const overrideKeys = new Set(
    override.filter(h => h.enabled !== false && h.key).map(h => h.key.toLowerCase())
  );
  return [
    ...base.filter(h => h.enabled !== false && h.key && !overrideKeys.has(h.key.toLowerCase())),
    ...override,
  ];
}
...
export function resolveInheritedConfig(
  ...
  const variables = mergeKVArrays(colCfg.variables ?? [], folCfg.variables ?? []);
...
function buildHeaders(
  ...
  if (auth.type === "bearer" && auth.token) {
    ...
    result["Authorization"] = `Basic ${btoa(`${rv(auth.username)}:${rv(auth.password)}`)}`;
...
function buildQueryString(
  ...
  const urlObj = new URL(resolved.startsWith("http") ? resolved : `http://${resolved}`);
...
export function prepareProxyRequest(
  ...
  const effectiveAuth =
    draft.auth.type !== "none" ? draft.auth : (inherited?.auth ?? { type: "none" as const });
  ...
  if (baseUrl && !baseUrl.startsWith("http://") && !baseUrl.startsWith("https://") && !baseUrl.includes("{{")) {
    baseUrl = "https://" + baseUrl;
  }

  const rv = (s: string) => resolveVariables(s, env, scopeVars);

  const url = buildQueryString(baseUrl, draft.queryParams, effectiveAuth, env, scopeVars);
  ...
  if (draft.body.type === "json" || draft.body.type === "raw") {
    ...
    if (draft.body.type === "json" && !headers["Content-Type"] && !headers["content-type"]) {
  ...
  return { method: draft.method, url, headers, body: bodyContent, timeoutMs: 30000 };
}
