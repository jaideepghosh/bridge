...
function FolderRow({ folder, isExpanded, isSelected, onToggle, onSelect, onRename, onDelete }: {
  ...
  return (
    <div
      className={`w-full flex items-center px-2 py-1.5 text-xs hover:bg-sidebar-accent rounded-sm group transition-colors ${
...
export function Sidebar() {
  ...
  // Use a "collapsed" set — IDs in here are collapsed; everything else is expanded
  ...
  const handleNewFolder = (collectionId?: string) => {
    setNewFolderCollectionId(collectionId ?? collections[0]?.id);
