...
type EnvVarCandidate = {
  raw: string;         // e.g. "$TOKEN" or "${BASE_URL}"
...
function applyEnvConversions(parsed: ParsedCurl, candidates: EnvVarCandidate[]): ParsedCurl {
  ...
  const replace = (s: string) =>
    toConvert.reduce((acc, c) => acc.replaceAll(c.raw, `{{${c.name}}}`), s);
...
export function ImportDialog({ open, onClose }: Props) {
  const { openTab, environments, activeEnvironmentId, saveEnvironment } = useStore();
  ...
  // Keyboard shortcut inside the dialog: Enter when parsed and valid
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <kbd className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded font-mono">
                {navigator.platform?.includes("Mac") ? "⌘↵" : "Ctrl+↵"} to import
              </kbd>
            </div>
          </div>
              ...
              <div className="px-5 py-3 space-y-3 border-b bg-muted/10">
                ...
                <div className="grid grid-cols-3 gap-3 text-xs">
                  <div className="bg-muted/30 rounded-md px-3 py-2">
                    <div className="text-muted-foreground font-medium mb-0.5">Headers</div>
                    <div className="font-mono font-semibold">{parsed.headers.length}</div>
                  </div>
                  <div className="bg-muted/30 rounded-md px-3 py-2">
                    <div className="text-muted-foreground font-medium mb-0.5">Query params</div>
                    <div className="font-mono font-semibold">{parsed.queryParams.length}</div>
                  </div>
                  <div className="bg-muted/30 rounded-md px-3 py-2">
                    <div className="text-muted-foreground font-medium mb-0.5">Body</div>
                    <div className="font-mono font-semibold capitalize">{parsed.body.type}</div>
                  </div>
                </div>

                {parsed.auth.type !== "none" && (
                  <div className="text-xs flex items-center gap-1.5">
                    <span className="text-muted-foreground">Auth detected:</span>
                    <Badge variant="secondary" className="text-[10px] px-1.5">
                      {parsed.auth.type === "bearer" ? "Bearer Token" : parsed.auth.type === "basic" ? "Basic Auth" : "API Key"}
                    </Badge>
                  </div>
                )}
              </div>
