...
export function UrlInput({ value, onChange, onKeyDown, env, placeholder }: Props) {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLInputElement>) => {
    ...
    let foundVar: { name: string; resolvedValue: string | null } | null = null;

    for (const seg of segments) {
      const segWidth = ctx.measureText(seg.text).width;
      if (relX >= charX && relX <= charX + segWidth) {
        if (seg.isVar) {
          foundVar = { name: seg.varName, resolvedValue: resolveVar(seg.varName, env) };
        }
        break;
      }
      charX += segWidth;
    ...
    if (foundVar) {
      setHoveredVar({ name: foundVar.name, resolvedValue: foundVar.resolvedValue, x: e.clientX, y: e.clientY });
    } else {
      setHoveredVar(null);
    }
  ...
  return (
    <div ref={containerRef} className="relative flex-1">
      ...
      {/* Actual input — transparent text so overlay shows through; caret stays visible */}
      <input
        ref={inputRef}
        value={value}
        ...
        onBlur={() => { setFocused(false); setHoveredVar(null); }}
        ...
        >
          <div className="bg-popover border border-border rounded-md shadow-xl px-3 py-2 text-xs min-w-[180px]">
            ...
            {hoveredVar.resolvedValue != null ? (
              <div className="font-mono text-foreground break-all">{hoveredVar.resolvedValue}</div>
            ) : (
              <div className="text-muted-foreground italic">
                {env ? "Variable not found in environment" : "No environment selected"}
              </div>
            )}
            {env && (
              <div className="text-muted-foreground mt-1 text-[10px]">from {env.name}</div>
            )}
          </div>
        </div>
      )}

      {/* Variable summary bar shown when focused and there are variables */}
          ...
          {segments.filter(s => s.isVar).map((seg, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className={`font-mono truncate max-w-[200px] ${resolved ? "text-foreground" : "text-muted-foreground italic"}`}>
