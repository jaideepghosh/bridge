...
export function parsePostman(json: any): ImportedCollection | null {
  ...
  function traverse(items: PostmanItem[], parentFolderId: string | null = null) {
      ...
      if (Array.isArray(item.item)) {
        ...
        if (typeof req.url === "string") {
          ...
          if (!rawUrl && Array.isArray(req.url.host)) {
            const proto = (req.url as any).protocol ? `${(req.url as any).protocol}://` : "";
            const hostStr = req.url.host.join(".");
            const pathStr = Array.isArray(req.url.path) ? "/" + req.url.path.join("/") : "";
        ...
        if (req.body) {
          ...
          if (mode === "raw" && req.body.raw !== undefined) {
            const isJson = req.body.options?.raw?.language === "json" || req.body.raw.trim().startsWith("{") || req.body.raw.trim().startsWith("[");
            body = {
        ...
        if (req.auth) {
          ...
          const getParam = (params: PostmanAuthParam[] | undefined, key: string) =>
          ...
          if (authType === "bearer" && req.auth.bearer) {
            ...
            auth = {
              ...
              in: (getParam(req.auth.apikey, "in") || "header") === "query" ? "query" : "header",
            };
          }
        }
