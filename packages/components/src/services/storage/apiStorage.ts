...
export class ApiStorageProvider implements StorageProvider {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`[ApiStorageProvider] Failed to load data from API: ${response.statusText}. Using empty memory store.`);
        this.data = { ...EMPTY_DATA };
      }
    } catch (err) {
      console.error("[ApiStorageProvider] Failed to connect to storage API. Using empty memory store.", err);
      this.data = { ...EMPTY_DATA };
    }
  }
  ...
  private async flush(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        console.error(`[ApiStorageProvider] Flush failed: ${response.statusText}`);
      }
    } catch (err) {
  ...
  // Collections
  getCollections(): Collection[] { return this.data.collections; }
  saveCollection(collection: Collection): void {
    const idx = this.data.collections.findIndex(i => i.id === collection.id);
    if (idx >= 0) this.data.collections[idx] = collection; else this.data.collections.push(collection);
    this.flush();
  }
  ...
  saveFolder(folder: Folder): void {
    ...
    if (idx >= 0) this.data.folders[idx] = folder; else this.data.folders.push(folder);
    this.flush();
  }
  ...
  // Requests
  getRequests(): SavedRequest[] { return this.data.requests; }
  saveRequest(request: SavedRequest): void {
    const idx = this.data.requests.findIndex(i => i.id === request.id);
    if (idx >= 0) this.data.requests[idx] = request; else this.data.requests.push(request);
    this.flush();
  }
  ...
  // Examples
  getExamples(): ApiExample[] { return this.data.examples; }
  saveExample(example: ApiExample): void {
    const idx = this.data.examples.findIndex(i => i.id === example.id);
    if (idx >= 0) this.data.examples[idx] = example; else this.data.examples.push(example);
    this.flush();
  }
  ...
  // Environments
  getEnvironments(): Environment[] { return this.data.environments; }
  saveEnvironment(env: Environment): void {
    const idx = this.data.environments.findIndex(i => i.id === env.id);
    if (idx >= 0) this.data.environments[idx] = env; else this.data.environments.push(env);
    this.flush();
  }
  ...
  // Active environment
  getActiveEnvironmentId(): string | null { return this.data.activeEnvironmentId; }
