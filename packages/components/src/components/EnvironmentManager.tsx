...
export function EnvironmentManager() {
  ...
  const updateSelectedEnv = (updates: Partial<Environment>) => {
    ...
    saveEnvironment({ ...selectedEnv, ...updates, updatedAt: new Date().toISOString() });
  };
  ...
  const removeVariable = (id: string) => {
    if (!selectedEnv) return;
    updateSelectedEnv({ variables: selectedEnv.variables.filter(v => v.id !== id) });
  };
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-64 border-r flex flex-col bg-muted/30">
              <div className="p-2 border-b">
                <Button variant="outline" size="sm" className="w-full justify-start text-xs" onClick={handleCreateEnv}>
                ...
                {environments.map(env => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm rounded-md transition-colors ${
                      selectedEnvId === env.id ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                    }`}
                  >
                    {env.name}
                  </button>
              ...
              {selectedEnv ? (
                <>
                  <div className="p-4 border-b flex justify-between items-center bg-card">
                    ...
                    />
                    <Button variant="destructive" size="sm" onClick={() => deleteEnvironment(selectedEnv.id)}>
                  ...
                  <div className="flex-1 overflow-y-auto p-4">
                    <table className="w-full text-sm">
                      ...
                      <tbody className="space-y-1">
                        {selectedEnv.variables.map(v => (
                          <tr key={v.id} className="group border-b last:border-0">
                            <td className="py-1">
                              <input
                                type="checkbox"
                                checked={v.enabled}
                            ...
                            <td className="py-1 text-center">
                              <button onClick={() => updateVariable(v.id, { isSecret: !v.isSecret })} className="text-muted-foreground hover:text-foreground">
                                {v.isSecret ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                              </button>
                            </td>
                            <td className="py-1 text-right">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100" onClick={() => removeVariable(v.id)}>
                    ...
                    <Button variant="ghost" size="sm" className="mt-4 text-xs" onClick={addVariable}>
