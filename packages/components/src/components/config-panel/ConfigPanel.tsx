...
function KVTable({
  ...
  const remove = (id: string) =>
    onChange && onChange(items.filter((i) => i.id !== id));

  return (
    <div
      className={`border rounded-md overflow-hidden bg-card ${readOnly ? "opacity-60" : ""}`}
    >
            ...
            <div className="p-1 flex items-center justify-center">
              {!isEmpty && (
                <input
                  ...
                  onChange={(e) =>
                    !readOnly && update(item.id, { enabled: e.target.checked })
                  }
                  disabled={readOnly}
...
function AuthEditor({
  ...
  const handleTypeChange = (v: string) => {
    ...
    if (v === "none") onChange({ type: "none" });
      ...
      onChange({ type: "apiKey", key: "X-API-Key", value: "", in: "header" });
  };

  return (
    <div
      className={`max-w-md space-y-4 ${readOnly ? "opacity-60 pointer-events-none" : ""}`}
    >
      <Select
        value={auth.type}
      ...
      {auth.type === "bearer" && (
        <div className="space-y-1.5">
          ...
          <Input
            value={auth.token}
            onChange={(e) =>
              onChange && onChange({ type: "bearer", token: e.target.value })
            }
            readOnly={readOnly}
        ...
        <div className="space-y-2">
          <div className="space-y-1.5">
            ...
            <Input
              value={auth.username}
              onChange={(e) =>
                onChange && onChange({ ...auth, username: e.target.value })
              }
              readOnly={readOnly}
              className="font-mono text-sm"
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              value={auth.password}
              onChange={(e) =>
                onChange && onChange({ ...auth, password: e.target.value })
              }
              readOnly={readOnly}
              className="font-mono text-sm"
        ...
        <div className="space-y-2">
          <div className="space-y-1.5">
            ...
            <Input
              value={auth.key}
              onChange={(e) =>
                onChange && onChange({ ...auth, key: e.target.value })
              }
              readOnly={readOnly}
              className="font-mono text-sm"
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={auth.value}
              onChange={(e) =>
                onChange && onChange({ ...auth, value: e.target.value })
              }
              readOnly={readOnly}
              className="font-mono text-sm"
...
const TAB_TRIGGER =
  "rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:shadow-none px-0 py-2 text-xs";
...
export function ConfigPanel() {
  ...
  const folder = !isCollection
    ? folders.find((f) => f.id === selectedSidebarItem.id)
    : undefined;
  ...
  const parentConfig: ScopeConfig | undefined = folder
    ? (collection?.config ?? {})
    : undefined;
    ...
    <div className="flex flex-col h-full bg-background overflow-hidden">
      ...
      >
        <div className="px-4 border-b shrink-0">
          <TabsList className="bg-transparent h-10 p-0 space-x-6">
            ...
            <TabsTrigger value="variables" className={TAB_TRIGGER}>
              Variables
              {variables.filter((v) => v.enabled && v.key).length > 0
        ...
        <div className="flex-1 overflow-y-auto">
          <TabsContent
            value="overview"
            className="p-4 m-0 border-0 flex flex-col h-full overflow-hidden"
          >
          ...
          <TabsContent value="auth" className="p-4 m-0 border-0">
            ...
            <AuthEditor
              auth={auth}
              onChange={(a) => updateConfig({ auth: a })}
            />
            {parentAuth && parentAuth.type !== "none" && (
              <InheritedSection
                label={`Inherited from "${collection?.name}": ${AUTH_LABEL[parentAuth.type] ?? parentAuth.type}`}
              >
          ...
          <TabsContent value="headers" className="p-4 m-0 border-0">
            ...
            <KVTable
              items={headers}
              onChange={(h) => updateConfig({ headers: h })}
              placeholderKey="Header Name"
              placeholderValue="Header Value"
            />
            {parentHeaders.filter((h) => h.key).length > 0 && (
              <InheritedSection
                label={`${parentHeaders.filter((h) => h.enabled && h.key).length} header(s) inherited from "${collection?.name}"`}
              >
                <KVTable
                  items={parentHeaders}
                  readOnly
          ...
          <TabsContent value="variables" className="p-4 m-0 border-0">
            ...
            {parentVariables.filter((v) => v.key).length > 0 && (
              <InheritedSection
                label={`${parentVariables.filter((v) => v.enabled && v.key).length} variable(s) inherited from "${collection?.name}"`}
              >
                <KVTable
                  items={parentVariables}
                  readOnly
                  placeholderKey="Variable Name"
                  placeholderValue="Value"
                />
